...
float FloatImage::smartAccessor(int x, int y, int z, bool clampToEdge) const {
    ...
    if (0 <= x  && x < m_sizeX && 0 <= y && y < m_sizeY && 0 <= z && z < m_sizeZ) {
    ...
    // return closet available pixel
    else {
        int temp_x = clamp(x, 0, m_sizeX - 1);
        int temp_y = clamp(y, 0, m_sizeY - 1);
        int temp_z = clamp(z, 0, m_sizeZ - 1);
        return m_data[temp_z*m_strideZ + temp_y*m_sizeX + temp_x];
    }
}
